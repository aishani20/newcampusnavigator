...
const Navbar = () => {
  ...
  function handleClickOutside(event) {
    if (!event.target.closest(".bell-menu") && !event.target.closest(".account-menu")) {
  ...
  function toggleNotifications() {
    ...
    // Close account menu if notifications are opened
  ...
  function toggleMenu() {
    setShowMenu(!showMenu);
    // Close notifications if account menu is opened
    if (showMenu) {
      setShowNotifications(false);
    }
  }
  ...
  return (
    <div className={`border-b-2  pb-4 grid grid-cols-12 items-center relative dark:border-[#51585B] ${token === null ? "md:grid-cols-4 " : ""}`}>
      ...
      <div className={`text-[#3652DD] md:flex items-center justify-center transition duration-300 ease-in-out transform hover:scale-105 ${token === null ? "lg:col-start-9 md:col-start-10 col-end-12" : "lg:col-start-9 md:col-start-10 col-end-12"} ${showMenu === true ? "block sm:block absolute md:static sm:right-[-2.5rem] -right-5 top-80 sm:top-40 z-30" : "md:flex hidden"}`}>
        <Link to="/predict" onClick={() => setShowMenu(false)} className="flex items-center justify-center text-lg px-4 py-2 font-semibold border-2 border-[#3652DD] rounded-md">
          <img src={predictionImage} alt="Logo" className="w-7 h-7 mr-2" />
          <span className={`lg:block md:hidden ${showMenu === true ? "block" : ""}`}>Check Your Success</span>
        </Link>
      </div>
      ...
      {token !== null && (
        <div className="flex gap-3 relative justify-end items-center sm:col-start-11 sm:col-end-12 md:col-start-12 md:col-end-13 col-start-11 col-end-12">
          <VscBell
            className={`w-6 h-6 cursor-pointer sm:block dark:text-[#A2A19F] ${showMenu ? "" : ""}`}
            onClick={toggleNotifications} 
          />
          <VscAccount
            className={`w-6 h-6 cursor-pointer md:block dark:text-[#A2A19F] ${showMenu ? "hidden" : "hidden"}`}
            onClick={toggleMenu} 
          />
          {showMenu && (
            <div className={`flex flex-col border p-2 rounded shadow-sm absolute top-8 right-3 bg-white z-20 gap-1 text-sm account-menu dark:bg-[#17191A] dark:text-[#FFFFFF]`}>
              <span>Profile</span>
              <span>Setting</span>
              <span className={`flex items-center gap-1 cursor-pointer`} onClick={signoutHandler}>
                <p>Signout</p>
                <VscSignOut />
              </span>
            </div>
      ...
      {token === null && (
        <Link to={location.pathname === "/login" ? "/signup" : "/login"} className="flex justify-end col-start-12 col-end-13 md:ml-12">
          <div className={`text-[18px] bg-[#3652DD] px-[30px] py-[10px] text-white rounded-[4px] sm:block`}>{location.pathname === "/login" ? "SIGNUP" : "LOGIN"}</div>
        </Link>
      )}

      {token !== null && (
        <div className={`flex md:hidden col-start-12 col-end-13 ${showMenu === true ? "justify-center " : "justify-end"} relative`}>
          <div onClick={menuToggleHandler}>
            {showMenu === false ? (
              <GiHamburgerMenu className={`cursor-pointer w-7 h-7 dark:text-[#A2A19F] ${showMenu && "bg-gray-400 p-1"} z-40`} />
            ) : (
              <AiOutlineClose className={`cursor-pointer w-8 h-8 ${showMenu && "bg-gray-400 p-1"} z-40`} />
            )}
          </div>
